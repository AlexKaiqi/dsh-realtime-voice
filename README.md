...
The Client service owns `RTCPeerConnection`, data channels, microphone tracks, media/audio elements, WebSockets, `AudioContext`, `AudioWorklet` PCM capture and playback, `SpeechRecognition`, `speechSynthesis`, listeners, late-callback guards, and the neutral tool-execution loop. It does not know drafts, Agent submission, knowledge bases, pets, or any product business semantics. The Doubao input worklet is loaded from the same Host base path; it also provides conservative browser-side barge-in detection so sustained user speech stops queued output immediately while Provider VAD remains the authoritative turn boundary. Cancelled response audio stays suppressed until the next response begins. No deprecated `ScriptProcessorNode`, Blob module, or production microphone bypass is retained.
